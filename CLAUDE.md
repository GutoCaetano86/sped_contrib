...
- **O limite real de upload não é do código, é da Vercel**: toda função serverless (o que inclui `/api/upload`) tem corpo de requisição limitado a uns 4,5 MB, e um EFD real passa disso fácil (o arquivo de aceite tem 17 MB). A rota `/api/upload` não recebe mais o arquivo — o cliente sobe direto pro bucket `uploads` do Storage (URL assinada de upload) e só depois registra os metadados via API. Se `/api/upload` voltar a receber bytes de arquivo no corpo, o limite volta.
- **`<a download>` é ignorado pelo navegador quando o `href` é cross-origin** — e a signed URL do Storage sempre é (`*.supabase.co` contra o domínio do app), então definir `link.download` no cliente não faz nada sozinho. Quem força o download é o servidor, pedindo `Content-Disposition: attachment` na própria assinatura: `createSignedUrl(caminho, segundos, { download: nome })` (`nomeParaDownload` em `Dependencias.urlAssinada`). Sem isso o Storage serve TXT como `text/plain` e ele abre numa aba, e qualquer link cross-origin cai na mesma pegadinha, não só download de arquivo.
- **A resposta de "reenviar confirmação de cadastro" é ambígua de propósito** (mesma frase exista ou não conta com aquele e-mail, esteja ela confirmada ou não): diferenciar os casos vazaria pra quem pede se um e-mail tem conta no sistema. A única exceção segura é o rate limit do próprio GoTrue, que tem dois patamares — 60 s por e-mail (mensagem com contagem regressiva) e, depois, um teto por **hora** do SMTP embutido do Supabase que é por **projeto**, não por usuário, e por isso fácil de esgotar em produção com poucos cadastros seguidos. Não tem correção em código para o segundo — a saída é configurar SMTP próprio no painel, e até lá é pendência de infra igual à do Leaked Password Protection (seção "Banco e segurança" acima).
